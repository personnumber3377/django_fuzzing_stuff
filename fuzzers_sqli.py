...
expression_model_names = [
    ...
    "Author", "Publisher", "Book", "Store", "Employee_aggregation", "DTModel", "FileFieldThing", "Car", "Supply"
] # Add car too maybe here???
...
'''
This function here tries to pick the thing dynamically...
'''


def get_models_with_filefields():
    models_with_files = []
    # expression_model_names
    # print("globals(): "+str(globals()))
    models = [globals()[x] for x in expression_model_names]
    for model in models: # apps.get_models():
        for field in model._meta.get_fields():
            if isinstance(field, (FileField, ImageField)):
                models_with_files.append(model)
                break  # No need to check more fields in this model
    return models_with_files

print("Here is the bullshit: "+str(get_models_with_filefields()))

def vuln_model_fields(test_string: str):
    # print("Fuck here!!!!!!!!!!!")
    if len(test_string) <= 2:
        return None
    model, test_string = pick_model(test_string)
    for field in model._meta.get_fields():
        if not (field.concrete and not field.auto_created):
            continue
        field_name = field.name

        model.objects.filter(**{field_name: test_string}).exists()
        '''
        try:
            # Test direct value filtering
            model.objects.filter(**{field_name: test_string}).exists()
        except Exception as e:
            print(f"Crash in field {field_name}: {repr(e)}")

        try:
            # Test each lookup (like __icontains, __lt, etc.)
            for lookup in field.get_lookups():
                lookup_expr = f"{field_name}__{lookup}"
                model.objects.filter(**{lookup_expr: test_string}).exists()
        except Exception as e:
            print(f"Crash in lookup {lookup_expr}: {repr(e)}")
...
vuln_stuff = [vuln_extract, vuln_trunc, vuln_f_expression, vuln_coalesce, vuln_concat, vuln_json_object, json_contains_shit, json_has_key, vuln_model_fields] # These are the function stuff shit...
...
def fuzz_sqli(data: bytes): # Fuzz for SQL injection
    ...
    try:
        ...
        if "Column aliases cannot contain whitespace" not in str(e) and "conflicts with a field on " not in str(e) and "Invalid option name" not in str(e) and "Unknown options" not in str(e) and "must be True or False" not in str(e) and "but got" not in str(e):
            raise
        ...
        # ValidationError: ['“ge” value has an invalid format. It must be in HH:MM[:ss[.uuuuuu]] format.']
        # ValidationError: ['“1:81” value has the correct format (HH:MM[:ss[.uuuuuu]]) but it is an invalid time.']

        if "UUID" not in str(e) and "It must be in" not in str(e) and "has the correct format" not in str(e):
            raise
    except ProgrammingError as e:
        if "Incorrect number of" not in str(e):
            raise
